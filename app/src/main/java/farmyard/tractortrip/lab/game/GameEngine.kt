...
import farmyard.tractortrip.lab.TractorPerks
...
class GameEngine(
    ...
    fun queueTurn(direction: Direction) {
        ...
        if (active.blockedOnCrane) {
            val targetCell = Cell(active.row + direction.dr, active.col + direction.dc)
            if (level.isWalkable(targetCell)) {
                active.direction = direction
                active.incomingDirection = direction
    ...
    fun step(): Boolean {
        ...
        tractors.forEachIndexed { index, tractor ->
            ...
            val options = forwardExits(tractor.row, tractor.col, incoming)
            val nextDir = resolveExit(options, tractor.row, tractor.col, incoming, pending)
            if (index == activeIndex && options.size > 1) tractor.pendingTurn = null
    ...
    companion object {
        ...
        const val FUEL_DRAIN_PER_SEC = 100f / 180f
